...
function suggestFollowUpStatus(followUps, currentMessage) {
  ...
  } else if (/(?:还在|仍在|继续)?(?:等|等待|等着).*(?:结果|回复|消息|通知)|还没.*(?:结果|回复|消息|通知)/u.test(text)) {
    suggestedStatus = "waiting";
    reason = "你似乎仍在等待这件事的结果";
  } else if (/(?:先|暂时|暂且).*(?:不聊|放一放|搁置|暂停|不做)|以后再说/u.test(text)) {
    suggestedStatus = "paused";
    reason = "你似乎想先把这件事放一放";
  }

  if (!suggestedStatus || suggestedStatus === item.status) return null;
  return { followUpId: item.id, title: item.title, currentStatus: item.status, suggestedStatus, reason };
}
